...
from tests.schemas import (get_schema_without_enum,
...
def test_delete_enum_render(connection: 'Connection'):
    ...
    old_enum_variants = ["active", "passive"]  # fixme Why it has old_ prefix?
    ...
    op.add_column('{USER_TABLE_NAME}', sa.Column('{USER_STATUS_COLUMN_NAME}', postgresql.ENUM({', '.join(map(repr, old_enum_variants))}, name='{USER_STATUS_ENUM_NAME}', create_type=False), autoincrement=False, nullable=True))
    # ### end Alembic commands ###""")
...
def test_create_enum_on_create_table_render(connection: 'Connection'):
    """Check that library correctly creates enum before its use inside add_column"""
    new_enum_variants = ["active", "passive"]

    database_schema = get_schema_with_enum_variants(new_enum_variants)

    context = create_migration_context(connection, database_schema)
...
def test_create_enum_on_drop_table_render(connection: 'Connection'):
    """Check that library correctly creates enum before its use inside add_column"""
...
def test_create_enum_render_with_array(connection: 'Connection'):
    """Check that library correctly creates enum before its use inside add_column"""
...
def test_delete_enum_render_with_array(connection: 'Connection'):
    """Check that library correctly removes unused enum"""
    ...
    assert (template_args["upgrades"] ==
            f"""# ### commands auto generated by Alembic - please adjust! ###
    op.drop_column('{CAR_TABLE_NAME}', '{CAR_COLORS_COLUMN_NAME}')
    sa.Enum({', '.join(map(repr, old_enum_variants))}, name='{CAR_COLORS_ENUM_NAME}').drop(op.get_bind())
    # ### end Alembic commands ###""")
    # For some reason alembic decided to add redundant autoincrement=False on downgrade
    assert (template_args["downgrades"] ==
            f"""# ### commands auto generated by Alembic - please adjust! ###
    sa.Enum({', '.join(map(repr, old_enum_variants))}, name='{CAR_COLORS_ENUM_NAME}').create(op.get_bind())
    op.add_column('{CAR_TABLE_NAME}', sa.Column('{CAR_COLORS_COLUMN_NAME}', postgresql.ARRAY(postgresql.ENUM({', '.join(map(repr, old_enum_variants))}, name='{CAR_COLORS_ENUM_NAME}')), autoincrement=False, nullable=True))
    # ### end Alembic commands ###""")
