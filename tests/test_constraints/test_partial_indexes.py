...
def test_error_when_dropping_enum_value_referenced_in_index(connection: "Connection"):
    """
    Test that we get a clear error message when trying to drop an enum value
    that is referenced in a partial index.
    """
    ...
    # Try to drop the 'deleted' value - this should fail with a clear error
    ...
    assert "Cannot drop enum value(s) 'deleted'" in error_message
    assert "partial index 'idx_not_deleted'" in error_message
...
def test_fallback_detection_of_partial_indexes_with_enum_expressions(connection: "Connection"):
    ...
    # Extract index names
    index_names = {idx_name.split('.')[-1] for idx_name, _ in dependent_indexes}
    ...
    for idx_name, idx_def in dependent_indexes:
        if 'idx_high_priority_tasks' in idx_name:
            assert "'high'::task_priority" in idx_def or "'high'::public.task_priority" in idx_def
        elif 'idx_urgent_tasks' in idx_name:
            assert "IN" in idx_def and "'urgent'" in idx_def
        elif 'idx_active_tasks' in idx_name:
            assert "'low'" in idx_def and ("<>" in idx_def or "!=" in idx_def)
        elif 'idx_medium_priority' in idx_name:
            assert "'medium'" in idx_def
    ...
    public_index_names = {idx_name for idx_name, _ in public_indexes}
    ...
    other_index_names = {idx_name for idx_name, _ in other_indexes}
