...
def test_sync_enum_values_unique_column(connection: 'Connection'):
    ...
    Table(
        ...
        Column('role', postgresql.ENUM(*old_enum_variants, name='user_role'),
               nullable=True),
        Index('admin_unique', 'role', unique=True, postgresql_where=sqlalchemy.text(f"role = 'admin'::user_role"))
    )
    ...
    ops.sync_enum_values(DEFAULT_SCHEMA, 'user_role', new_enum_variants,
